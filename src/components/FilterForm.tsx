...
const FilterForm: React.FC<FilterFormProps> = ({
  ...
  const renderFilterField = (filter: FilterField): React.ReactElement => {
    const value =
      filterValues[filter.name] || (filter.type === 'multiselect' ? [] : '');
          ...
          <FormControl fullWidth>
            ...
            <Select
              ...
              onChange={(e: SelectChangeEvent<string[]>) =>
                handleFilterChange(filter.name, e.target.value as string[])
              }
              label={filter.label}
              data-testid={
                TestIds.filterForm[
  ...
  return <Card sx={{ mb: 3 }}>{filterContent}</Card>;
};
