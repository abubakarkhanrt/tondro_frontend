...
import {
  ...
  Grid, Alert, CircularProgress, Box, SelectChangeEvent
} from '@mui/material';
...
const CreateDialog: React.FC<CreateDialogProps> = ({ 
  ...
  const validateForm = (): boolean => {
    ...
    fields.forEach(field => {
      ...
      if (field.maxLength && value && value.length > field.maxLength) {
        errors[field.name] = `${field.label} must be no more than ${field.maxLength} characters`;
      }
    });
      ...
      case 'select':
        return (
          <FormControl fullWidth error={hasError}>
            ...
            <Select
              value={value}
              onChange={(e: SelectChangeEvent) => handleInputChange(field.name, e.target.value)}
              label={field.label}
  ...
  return (
    ...
    >
      <DialogTitle data-testid={TestIds.createDialog.title}>{title}</DialogTitle>
      <DialogContent>
        <Box sx={{ pt: 1 }}>
          {error && (
            <Alert severity="error" sx={{ mb: 2 }} data-testid={TestIds.createDialog.error}>
              {error}
            </Alert>
          )}
