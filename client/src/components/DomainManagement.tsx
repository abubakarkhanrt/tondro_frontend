...
const DomainManagement: React.FC<DomainManagementProps> = ({ 
  ...
  const handleUpdateDomain = async (domainId: string, formData: UpdateDomainRequest) => {
    try {
      await apiHelpers.updateDomain(domainId, formData);
      setSnackbar({ open: true, message: 'Domain updated successfully', severity: 'success' });
            ...
            <TableBody>
              {Array.isArray(domains) && domains.map(domain => (
                <TableRow key={domain.id}>
                  ...
                  <TableCell>
                    <Chip
                      label={Boolean(domain.is_primary) ? 'Primary' : 'Secondary'}
                      color={Boolean(domain.is_primary) ? 'primary' : 'default'}
                      size="small"
                    />
                  </TableCell>
                  <TableCell>
                    <Chip
                      label={domain.status}
                      color={getStatusColor(theme, domain.status)}
                      size="small"
                    />
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <Box sx={{ display: 'flex', gap: 1 }}>
                      ...
                      <Tooltip title="Delete Domain">
                        {Boolean(domain.is_primary) && Array.isArray(domains) && domains.filter(d => Boolean(d.is_primary)).length === 1 ? (
                          <span>
                            <IconButton
                              size="small"
                              disabled={true}
                          ...
                          <IconButton
                            size="small"
                            onClick={() => {
                              if (window.confirm(`Delete domain "${domain.domain_name || domain.name}"?`)) {
        ...
        <Grid container spacing={2} sx={{ mt: 1 }}>
          <Grid item xs={12}>
            <TextField
              ...
              error={!!errors.name}
              helperText={errors.name || 'Enter domain name (e.g., company.com)'}
              disabled={loading}
