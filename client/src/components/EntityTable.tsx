...
import {
  ...
  Paper, Button, IconButton, Chip, TablePagination, Box, Typography,
  CircularProgress, Alert, Tooltip, Collapse, SelectChangeEvent, useTheme
} from '@mui/material';
...
interface TableColumn {
  key: string;
  label: string;
  type?: 'text' | 'date' | 'datetime' | 'boolean' | 'chip' | 'status' | 'currency' | 'number' | 'email' | 'url';
...
const EntityTable: React.FC<EntityTableProps> = ({
  ...
  const renderCellValue = (value: any, column: TableColumn): React.ReactNode => {
      ...
      case 'status':
        return (
          <Chip 
            label={value} 
            style={{ 
              backgroundColor: getStatusBackgroundColor(theme, value),
              color: '#ffffff'
            }}
            size="small"
          />
  ...
  const renderActions = (row: any, column: TableColumn): React.ReactElement | null => {
        ...
        {column.actions.map((action, index) => (
          <Tooltip key={index} title={action.tooltip || action.label}>
            <Button
              ...
              data-testid={TestIds.entityTable.actionButton(action.label.toLowerCase().replace(/\s+/g, '-'), row.id)}
            >
              {action.label}
            </Button>
          </Tooltip>
  ...
  const renderExpandableContent = (row: any, column: TableColumn): React.ReactElement | null => {
  ...
  if (error) {
    return (
      <Paper>
        <Alert severity="error" sx={{ m: 2 }} data-testid={TestIds.common.errorAlert}>
          {error}
        </Alert>
      </Paper>
    );
            ...
            <TableRow>
              {columns.map((column) => (
                <TableCell 
                  ...
                  data-testid={column.sortable ? TestIds.entityTable.sortColumn(column.key) : undefined}
                >
                  {column.label}
            ...
            {sortedData.length === 0 ? (
              ...
              sortedData.map((row) => (
                <React.Fragment key={row.id}>
                  ...
                  {expandableRows && renderExpandableContent(row, columns.find(col => col.expandable)!)}
                </React.Fragment>
              ))
            )}
          </TableBody>
