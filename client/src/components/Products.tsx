...
import {
  ...
  TablePagination, Snackbar, Select, MenuItem, FormControl, InputLabel, SelectChangeEvent,
  IconButton, Chip
} from '@mui/material';
import { 
...
const Products: React.FC = () => {
  ...
  const handleCreateProduct = async (formData: CreateProductRequest): Promise<void> => {
    try {
  ...
  const handleDeleteProduct = async (id: string): Promise<void> => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: 'Product deletion not implemented in API', severity: 'warning' });
    } catch (error) {
  ...
  const handleUpdateProduct = async (formData: UpdateProductRequest): Promise<void> => {
                      ...
                      <TableCell>
                        <Box sx={{ display: 'flex', gap: 1 }}>
                          <IconButton
                            ...
                            data-testid={TestIds.products.viewDetails(product.id)}
                          >
                            <VisibilityIcon />
                          </IconButton>
                          <IconButton
