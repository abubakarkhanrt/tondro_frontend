...
const Organizations: React.FC = () => {
  ...
  const fetchOrganizations = useCallback(async (): Promise<void> => {
    ...
    setOrganizations(prev => ({ ...prev, loading: true, error: '', abortController }));
    ...
    try {
      ...
      const response = await apiHelpers.getOrganizations(params, abortController.signal);
  ...
  const handleCreateOrganization = useCallback(async (formData: CreateOrganizationRequest): Promise<void> => {
    try {
  ...
  const handleDeleteOrganization = useCallback(async (id: string, force: boolean = false): Promise<void> => {
    if (!confirm(`Are you sure you want to delete this organization?${force ? ' This action cannot be undone.' : ''}`)) {
      return;
    }

    try {
      await apiHelpers.deleteOrganization(id, force);
      setSnackbar({
        open: true,
  ...
  }, [fetchOrganizations, setSnackbar]);

  const handleUpdateStatus = useCallback(async (id: string, status: string): Promise<void> => {
    try {
      await apiHelpers.updateOrganizationStatus(id, status);
      setSnackbar({
        open: true,
  ...
  const handleFilterChange = useCallback((newFilters: { status: string; search: string }): void => {
  ...
  const handleUpdateOrganization = useCallback(async (formData: UpdateOrganizationRequest): Promise<void> => {
    if (!selectedOrg) return;

    try {
      await apiHelpers.updateOrganization(selectedOrg.organizationId, formData);
      setSnackbar({
        open: true,
  ...
  const FilterSection: React.FC<FilterSectionProps> = ({ filters, onFiltersChange, onClearFilters }) => {
  ...
  const OrganizationsTable: React.FC<{ products: Product[] }> = ({ products }) => (
                    ...
                    <TableRow key={org.organizationId}>
                      ...
                      <TableCell>
                        <Chip
                          label={org.status}
                          style={{ 
                            backgroundColor: getStatusBackgroundColor(theme, org.status),
                            color: '#ffffff'
                          }}
                          size="small"
                        />
                        ...
                        {org.subscriptions && org.subscriptions.length > 0 ? (
                          <Box>
                            {org.subscriptions.map((sub, index) => (
                              <Card key={index} sx={{ mb: 1, p: 1, backgroundColor: 'grey.50' }}>
                                <Grid container spacing={1} alignItems="center">
                                  ...
                                  <Grid item xs={4}>
                                    ...
                                    <Typography variant="body2" component="div">
                                      {(sub as any).current_usage || 0} / {(sub as any).usage_limit || 'Unlimited'}
                                    </Typography>
                                  </Grid>
                                </Grid>
                              </Card>
                        ...
                        <Box sx={{ display: 'flex', gap: 1 }}>
                          <Tooltip title="View Details">
                            <IconButton
                              ...
                              data-testid={TestIds.organizations.viewDetails(org.organizationId)}
                            >
                              <VisibilityIcon />
                            </IconButton>
                          </Tooltip>
                          <Tooltip title="Edit Organization">
                            <IconButton
                              ...
                              data-testid={TestIds.organizations.edit(org.organizationId)}
                            >
                              <EditIcon />
                            </IconButton>
                          </Tooltip>
...
const SubscriptionForm: React.FC<SubscriptionFormProps> = ({
  ...
  const updateSubscription = (index: number, field: keyof ProductSubscriptionRequest, value: any) => {
    ...
    <Box>
      ...
      {subscriptions.map((subscription, index) => (
        <Card key={index} sx={{ mb: 2, p: 2 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
            <Typography variant="subtitle1">Subscription {index + 1}</Typography>
            <IconButton
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} sm={6}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) => updateSubscription(index, 'product_id', e.target.value)}
                  label="Product"
                >
            ...
            <Grid item xs={12} sm={6}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) => updateSubscription(index, 'tier_name', e.target.value)}
                  label="Tier"
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                onChange={(e) => updateSubscription(index, 'ends_at', e.target.value)}
                fullWidth
              ...
              <FormControlLabel
                control={
                  <Switch
                    ...
                    onChange={(e) => updateSubscription(index, 'auto_renewal', e.target.checked)}
                  />
                }
                label="Auto Renewal"
              />
...
const CreateOrganizationDialog: React.FC<CreateOrganizationDialogProps> = ({ 
  ...
  const handleSubscriptionsChange = (subscriptions: ProductSubscriptionRequest[]) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.organizationDomain?.trim()) {
      ...
      newErrors.organizationDomain = 'Please enter a valid domain name (e.g., company.com)';
    }
    ...
    if (formData.initialSubscriptions.length === 0) {
      ...
      if (invalidSubscriptions.length > 0) {
        newErrors.subscriptions = 'Please complete all subscription details (product and tier are required)';
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const userFriendlyMessage = ERROR_MESSAGES[errorMessage as keyof typeof ERROR_MESSAGES] || errorMessage;
      
      // Set specific field errors if available
      if (errorMessage.includes('domain') || errorMessage.includes('Domain')) {
        setErrors(prev => ({ ...prev, organizationDomain: userFriendlyMessage }));
      } else {
      ...
      <DialogContent>
        <Box sx={{ pt: 1 }}>
          ...
          <TextField
            ...
            helperText={errors.organizationDomain || 'Enter a unique domain name for the organization'}
          />
          <TextField
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} sm={6}>
          ...
          <Chip
            ...
            style={{ 
              backgroundColor: getStatusBackgroundColor(theme, organization.status),
              color: '#ffffff'
            }}
            size="small"
          />
        ...
        <Grid item xs={12}>
          ...
          {organization.subscriptions && organization.subscriptions.length > 0 ? (
            <Box>
                  ...
                  <Grid container spacing={1}>
                    <Grid item xs={6}>
                      ...
                      <Typography variant="body1" component="div">
                        {(sub as any).product_name || `Product ID: ${sub.product_id}`}
                      </Typography>
                    </Grid>
                    <Grid item xs={6}>
                      <Typography variant="body2" color="text.secondary" component="div">
                      ...
                      <Chip
                        label={formatTierName((sub as any).tier || sub.tier_name)}
                        size="small"
                    ...
                    <Grid item xs={6}>
                      ...
                      <Chip
                        ...
                        size="small"
                        color={(sub as any).status === 'active' ? 'success' : 'default'}
                      />
                    </Grid>
                    <Grid item xs={6}>
                      <Typography variant="body2" color="text.secondary" component="div">
                    ...
                    <Grid item xs={6}>
                      ...
                      <Typography variant="body1" component="div">
                        {(sub as any).ends_at ? new Date((sub as any).ends_at).toLocaleDateString() : 'No end date'}
                      </Typography>
                    </Grid>
                  </Grid>
                </Card>
